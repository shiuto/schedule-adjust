...
import {
  ...
  List, Plus, Filter, X, Menu, AlignLeft,
} from 'lucide-react';
...
export function EventDetailPage() {
  ...
  const eventPersons = getEventPersons(event.id);
    ...
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      <header className="bg-white border-b shadow-sm z-30 shrink-0">
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          >
            <ArrowLeft size={15} />
            <span className="hidden sm:inline text-xs">一覧</span>
          </button>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {/* Date tabs */}
            <div className="flex items-center bg-gray-100 rounded-lg p-0.5 overflow-x-auto no-scrollbar max-w-[200px] sm:max-w-none">
              {event.dates.map((d) => (
                <button
                  key={d}
                  onClick={() => setCurrentDate(d)}
                  className={`px-2.5 py-1 text-xs rounded-md transition-all whitespace-nowrap font-medium ${
                    activeDate === d
            ...
            <div className="hidden sm:flex items-center bg-gray-100 rounded-lg p-0.5">
              <button
                ...
                className={`p-1.5 rounded-md transition-all ${viewMode === 'timeline' ? 'bg-white shadow text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}
                title="タイムライン"
              >
                <AlignLeft size={15} />
              </button>
            ...
            <button
              ...
              className={`hidden sm:flex items-center gap-1 px-2.5 py-1.5 text-xs rounded-lg border transition-all font-medium ${
                activeFilterCount > 0
            ...
            >
              <Filter size={12} />
              {activeFilterCount > 0 ? `絞込(${activeFilterCount})` : '絞込'}
            </button>

            {/* Add session */}
            <button
              onClick={() => setShowAddSession(true)}
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs sm:text-sm bg-blue-500 text-white rounded-lg hover:bg-blue-600 font-medium shadow-sm transition-colors"
            >
              <Plus size={14} />
        ...
        {/* Filter panel */}
        {showFilterPanel && (
          <div className="border-t bg-gradient-to-b from-gray-50 to-white px-4 py-3 animate-in slide-in-from-top-1">
            <div className="flex items-center justify-between mb-2.5">
              <span className="text-xs font-semibold text-gray-700">絞り込み設定</span>
              {activeFilterCount > 0 && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">人物</p>
                    ...
                    eventPersons.map((p) => (
                      <button
                        ...
                        className={`px-2.5 py-1 text-xs rounded-full border-2 transition-all font-medium ${
                          filterPersonIds.includes(p.id) ? 'border-transparent text-white shadow-sm' : 'border-gray-200 text-gray-600 bg-white hover:border-gray-300'
                        }`}
              ...
              <div>
                <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">エリア</p>
                <div className="flex flex-wrap gap-1.5">
                  {areas.length === 0 ? (
                    ...
                    areas.map((a) => (
                      <button
                        ...
                        className={`flex items-center gap-1 px-2.5 py-1 text-xs rounded-full border-2 transition-all font-medium ${
                          filterAreaIds.includes(a.id) ? 'border-transparent text-white shadow-sm' : 'border-gray-200 text-gray-600 bg-white hover:border-gray-300'
                        }`}
        ...
        {sidebarOpen && (
          <aside className="hidden sm:flex w-72 bg-white border-l flex-col shrink-0">
            <div className="flex border-b">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-sm font-medium transition-colors ${
                  sidebarTab === 'persons' ? 'text-blue-600 border-b-2 border-blue-500 bg-blue-50/30' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
                <Users size={14} /> 人物
              </button>
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-sm font-medium transition-colors ${
                  sidebarTab === 'venues' ? 'text-blue-600 border-b-2 border-blue-500 bg-blue-50/30' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
                <MapPin size={14} /> 会場
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-3">
              {sidebarTab === 'persons'
                ? <StakeholderPanel eventId={event.id} />
                : <VenuePanel eventId={event.id} />
              }
            </div>
          </aside>
        )}
          ...
          <div className="sm:hidden fixed inset-0 z-40 flex">
            ...
            <div className="w-72 bg-white flex flex-col shadow-2xl">
              <div className="flex items-center justify-between px-4 py-3 border-b">
                ...
                <button onClick={() => setMobileSidebarOpen(false)} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded-lg">
                  <X size={18} />
                </button>
              </div>
              <div className="flex border-b">
                <button
                  onClick={() => setSidebarTab('persons')}
                  className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-sm font-medium ${sidebarTab === 'persons' ? 'text-blue-600 border-b-2 border-blue-500' : 'text-gray-500'}`}
                >
                  <Users size={14} /> 人物
                </button>
                <button
                  onClick={() => setSidebarTab('venues')}
                  className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-sm font-medium ${sidebarTab === 'venues' ? 'text-blue-600 border-b-2 border-blue-500' : 'text-gray-500'}`}
                >
                  <MapPin size={14} /> 会場
                </button>
              </div>
      ...
      {showAddSession && (
        <SessionModal eventId={event.id} date={activeDate} onClose={() => setShowAddSession(false)} />
      )}
    </div>
  );
}
